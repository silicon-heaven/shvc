...
/** Result used by :c:member:`rpchandler_funcs.msg`. */
enum rpchandler_msg_res {
	 ...
	 * error because that is recorded in :c:type:`rpcclient` and thus handler's
	 * loop is terminated anyway. It is safe and preferred to just use
	 * :c:enumerator:`RPCHANDLER_MSG_DONE`.
	 */
	RPCHANDLER_MSG_STOP,
};

/** Context passed to the callbacks :c:member:`rpchandler_funcs.msg` */
...
/** Context passed to the callbacks :c:member:`rpchandler_funcs.ls` */
struct rpchandler_ls {
	 ...
	 * because you might not need to call :c:func:`rpchandler_ls_result` for
	 * every single node name when we are interested only in one of them. This
	 * function provides you that name.
	 *
	 * Note that if you plan to use it to compare against names one at a time
	 * then you can just use :c:var:`rpchandler_ls_result` that does exactly
...
/** Context passed to the callbacks :c:var:`rpchandler_funcs.dir` */
struct rpchandler_dir {
	 ...
	 * remarks such as for :c:var:`rpchandler_ls.name` apply here just with
	 * :c:func:`rpchandler_dir_result` function.
	 */
	const char *const name;
};

/** Context passed to the callbacks :c:var:`rpchandler_funcs.idle` */
struct rpchandler_idle {
	/** Last our activity. This is ``CLOCK_MONOTONIC`` time. */
	struct timespec last_send;
};

/** Result that can be returned from :c:var:`rpchandler_funcs.idle`.
 *
 ...
 * This will be propagated from :c:func:`rpchandler_idling` as error.
 */
#define RPCHANDLER_IDLE_STOP (INT_MIN)
/** Result that can be returned from :c:var:`rpchandler_funcs.idle`.
 *
 * This should be used if :c:func:`rpchandler_msg_new` returned ``NULL``. The
 * value is zero which causes idle to be run again as soon as possible (after
 * message retrieval is attempted).
 */
...
struct rpchandler_funcs {
	 ...
	 * array until some function returns :c:enumerator:`RPCHANDLER_MSG_DONE` or
	 * :c:enumerator:`RPCHANDLER_MSG_STOP`.
	 *
	 * This function must investigate :c:var:`rpchandler_msg.meta` to see if it
	 * should handle that message. It can immediately return ``false`` or it can
	 * proceed to handle it. It is prohibited to use
	 * :c:var:`rpchandler_msg.unpack` if ``false`` is returned.
	 *
	 ...
	 * needs to copy :c:struct:`rpcmsg_meta` for sending response later on. It
	 * is not allowed to pack any requests!
	 *
	 * The message parameters are parsed with :c:var:`rpchandler_msg.unpack` and
	 * :c:var:`rpchandler_msg.item`. Before you invoke first
	 * :c:macro:`cp_unpack` you must make sure that there is actually parameter
	 * to unpack with :c:func:`rpcmsg_has_value` (otherwise you can get unpack
	 * error even for valid message). Next you can use unpack functions to
	 * unpack parameters any way you wish.
	 *
	 ...
	 * for the given path.
	 *
	 * The implementation needs to call :c:func:`rpchandler_ls_result` and
	 * variants of that function for every node in the requested path.
	 *
	 * Note that this is attempted only if request for ``ls`` method is not
	 * handled by :c:var:`rpchandler_funcs.msg`.
	 */
	 ...
	 *
	 * The implementation needs to call :c:func:`rpchandler_dir_result` and
	 * variants of that function for every method on the requested path.
	 *
	 ...
	 * at that moment. It is preferred to use :c:macro:`RPCHANDLER_IDLE_LAST`
	 * macro to make this more explicit over just returning the negative number.
	 * The special value is the lowest negative number (it doesn't have positive
	 * counterpart), aliased with :c:macro:`RPCHANDLER_IDLE_STOP`, that is used
	 * to request handler's loop termination (the :c:func:`rpchandler_idling`
	 * will signal error).
	 */
...
struct rpchandler_stage {
	/** Pointer to the :c:struct:`rpchandler_funcs` with pointer to the function
 ...
 *   this client. It is an array that is ``NULL`` terminated (the
 *   :c:var:`rpchandler_stage.funcs` is zero). Stages are referenced as they are
 *   and thus you need to keep this pointer valid for the duration of object
 *   existence (or you can replace it with :c:func:`rpchandler_change_stages`).
 ...
 *   object existence.
 * :return: New RPC handler instance.
 */
...
/** Free all resources occupied by :c:type:`rpchandler_t` object.
 *
 * This is destructor for the object created by :c:func:`rpchandler_new`.
 *
 ...
 * :return: Pointer to the array of states.
 */
[[gnu::nonnull]]
 ...
 * it is not possible to change stages from withing the
 * :c:struct:`rpchandler_funcs` functions.
 *
 * :param handler: RPC Handler instance.
 ...
 * you should not use this directly because you must in between also call
 * :c:member:`rpchandler_idling`. These two steps are provided separatelly to
 * allow handler to be included in poll based even loops.
 *
 * :param rpchandler: RPC Handler instance.
 * :return: ``true`` if message handled (even by dropping) and ``false`` if
 ...
 *
 * This should be used in combination with :c:func:`rpchandler_next` if you are
 ...
 *
 * :param rpchandler: RPC Handler instance.
 * :return: The maximal time during which idle doesn't have to be called (unless
 *   new message is received). It is in milliseconds. Negative number can be
 *   returned that signals error and should be handled same as
 *   :c:macro:`RPCHANDLER_MSG_STOP` with :c:func:`rpchandler_next`.
 */
[[gnu::nonnull]]
...
/** Spawn thread that runs :c:func:`rpchandler_run`.
 *
 ...
 *   defaults.
 * :return: Integer value returned from ``pthread_create``.
 */
[[gnu::nonnull(1, 2)]]
 ...
 * released either with :c:func:`rpchandler_msg_send` or with
 * :c:func:`rpchandler_msg_drop`.
 *
 * This is implemented as macro that based on the ``HANDLER`` parameter expands
 * to an appropriate internal method call. Thanks to that it can be used for the
 * same usage in the different contexts:
 *
 * - :c:var:`rpchandler_funcs.msg` (:c:func:`rpchandler_msg`) can be used to
 *   send responses and signals when request message is received. This function
 *   always provides `NULL` if received message is either signal or response.
 * - :c:var:`rpchandler_funcs.idle` (:c:func:`rpchandler_idle`) can be used to
 *   send one message per invocation. This way you can send only one message per
 *   invocation and thus subsequent usages will provide ``NULL``.
 * - :c:var:`rpchandler_funcs.ls` and :c:var:`rpchandler_funcs.dir` are
 *   disallowed from sending messages and thus this won't work with
 *   :c:func:`rpchandler_ls` and :c:func:`rpchandler_dir`.
 * - Any function that is not called from :c:struct:`rpchandler_funcs` functions
 *   can use this to send messages.
 *
 * :param HANDLER: RPC Handler instance or context.
 * :return: Packer you need to use to pack message.
 */
 ...
 *
 * This calls :c:func:`rpcclient_sendmsg` under the hood and releases the lock
 * taken by :c:func:`rpchandler_msg_new`.
 *
 * :param HANDLER: RPC Handler instance or context.
 * :return: ``true`` if send was successful and ``false`` otherwise.
 */
 ...
 *
 * This calls :c:func:`rpcclient_dropmsg` under the hood and releases the lock
 * taken by :c:func:`rpchandler_msg_new`.
 *
 * :param HANDLER: RPC Handler instance or context.
 * :return: ``true`` if send was successful and ``false`` otherwise.
...
/** Utility combination of :c:macro:`rpchandler_msg_new` and
 * :c:macro:`rpcmsg_pack_request`.
 *
 * :param HANDLER: RPC Handler instance or context.
 ...
 * :return: :c:type:`cp_pack_t` or ``NULL`` on error.
 */
...
/** Utility combination of :c:macro:`rpchandler_msg_new` and
 * :c:macro:`rpcmsg_pack_request_void`.
 *
 * :param HANDLER: RPC Handler instance or context.
 * :param PATH: SHV path to the node the method we want to request is associated
 *   with.
