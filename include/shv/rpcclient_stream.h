...
enum rpcstream_proto {
	 ...
	 */
	RPCSTREAM_P_SERIAL,
	/** This is variant of :c:enumerator:`RPCSTREAM_P_SERIAL` that tracks and
	 * validates CRC32 of the transported messages to detect corrupted data.
	 */
...
struct rpcclient_stream_funcs {
	/** Called when :c:macro:`rpcclient_reset` is called and connection is not
	...
	/** Called when :c:macro:`rpcclient_disconnect` or
	 * :c:macro:`rpcclient_destroy` is called. The argument `destroy` signals if
	 * :c:var:`rpcclient_stream_funcs.connect` won't be called again and thus
	 * all reasources should be freed. Be aware that client might be
	 * disconnected before it is destryed and thus make sure that `fd` items are
	 * not ``-1``.
	 *
	...
	/** Called as propagation of :c:macro:`rpcclient_peername`. The provided
	 * ``fd`` are read and write file descriptors provided for convenience.
	 */
	size_t (*peername)(void *cookie, int fd[2], char *buf, size_t size);
	/** The result of the :c:macro:`rpcclient_contrack`. */
	bool contrack;
};
 ...
 *   established (in such case :c:var:`rpcclient_stream_funcs.connect` will be
 *   used). :c:macro:`rpcclient_destroy` closes this socket only if
 *   :c:var:`rpcclient_stream_funcs.connect` is not ``NULL``.
