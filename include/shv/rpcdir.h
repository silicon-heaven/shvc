 ...
 *
 * This is commonly combined with @ref RPCDIR_F_GETTER to signal that it might
 * not be efficient to call this method in advance to just potentially cache the
 * value.
 */
...
struct rpcdir {
	...
	const struct rpcdirsig {
		/*! Signal name */
		const char *name;
		/*! Parameter type description. It is `NULL` in case @ref rpcdir.result
		 * is the parameter to be provided.
		 */
		const char *param;
	}
