 ...
 * path. See :c:member:`rpcbroker_role.access`.
 *
...
struct rpcbroker_role {
	 ...
	 * reference counting.
	 */
	/** Assigned mount point or ``NULL`` if this client should not be
	 * accessible.
	 *
...
/** Result for the :c:type:`rpcbroker_login_t` callback. */
struct rpcbroker_login_res {
	/** This signals if :c:var:`rpcbroker_login_res.errmsg` was used (if
	 * ``false``) or :c:var:`rpcbroker_login_res.role` (if ``true``).
	 */
	bool success;
	/** */
	union {
		...
		/** Optional pointer to error message. It can be ``NULL`` if default
 ...
 * :param role: The role used by this client. The role must be valid all the
 *   time client stays registered. You must pass ``NULL`` if role is assigned by
 *   login.
 * :return: ID assigned to the newly registered client.
 */
[[gnu::nonnull(1, 2, 3, 4)]]
 ...
 *   integer previously returned either by :c:func:`rpcbroker_client_register`
 *   and not unregistered yet.
 */
[[gnu::nonnull]]
 ...
 * :param broker: Broker object.
 * :param client_id: The ID of the previously registered client. That is
 *   integer previously returned by :c:func:`rpcbroker_client_register` and not
 *   unregistered yet.
 * :return: The :c:type:`rpchandler_t` or ``NULL`` in case of invalid client ID.
 */
[[gnu::nonnull]]
 ...
 *
 * Call to this functions must be eventually followed by
 * :c:func:`rpcbroker_send_signal` or :c:func:`rpcbroker_drop_signal`.
 *
 * :param broker: Broker object.
 ...
 * :return: Context to be used for packing and released by
 *   :c:func:`rpcbroker_send_signal` or :c:func:`rpcbroker_drop_signal`.
 *   ``NULL`` is returned if this signal would not be propagated to any peer and
 *   thus should not be packed at all.
 */
[[gnu::nonnull(1, 2, 3, 4)]]
...
/** Send signal message.
 *
 * :param ctx: Context provided by :c:func:`rpcbroker_new_signal`.
 * :return: Boolean signaling if sending was successful.
 */
[[gnu::nonnull]]
 ...
 *
 * :param ctx: Context provided by :c:func:`rpcbroker_new_signal`.
 * :return: Boolean signaling if dropping was successful.
 */
[[gnu::nonnull]]
...
/** The state used in :c:func:`rpcbroker_run`. */
struct rpcbroker_state {
	...
	rpcserver_t *servers;
	/** Number of servers in :c:var:`rpcbroker_state.servers`. */
	size_t servers_cnt;
	 ...
	 * this function.
	 *
	 * This function must return client ID as returned by
	 * :c:func:`rpcbroker_client_register` or it call return negative number to
	 * signal that client wasn't registered.
	 */
	int (*new_client)(void *cookie, rpcbroker_t broker, rpcserver_t server,
		rpcclient_t client);
	/** Callback used to remove the client from the broker.
	 *
	 * This client was previously registered through
	 * :c:var:`rpcbroker_state.new_client`.
	 */
	void (*del_client)(void *cookie, rpcbroker_t broker, int client_id);
	/** The user specified pointer passed to the
	 * :c:var:`rpcbroker_state.new_client` and
	 * :c:var:`rpcbroker_state.del_client`.
	 */
	void *cookie;
};
 ...
 *
 * This is single-threaded implementation (you can use
 * :c:macro:`RPCBROKER_F_NOLOCK`).
 *
