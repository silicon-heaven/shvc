...
[[gnu::nonnull(1)]]
static inline struct user *config_get_user(struct config *conf, const char *name) {
	struct user ref = {.name = name ?: ""};
	return bsearch(&ref, conf->users, conf->users_cnt, sizeof ref, cmp_user);
}

[[gnu::nonnull(1)]]
static inline struct role *config_get_role(struct config *conf, const char *name) {
