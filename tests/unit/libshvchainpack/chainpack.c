...
static const struct {
	...
	{{.type = CP_ITEM_NULL}, B(0x80)},
	{{.type = CP_ITEM_BOOL, .as.Bool = true}, B(0xfe)},
	{{.type = CP_ITEM_BOOL, .as.Bool = false}, B(0xfd)},
	{{.type = CP_ITEM_INT, .as.Int = 0}, B(0x40)},
	{{.type = CP_ITEM_INT, .as.Int = -1}, B(0x82, 0x41)},
	{{.type = CP_ITEM_INT, .as.Int = -2}, B(0x82, 0x42)},
	{{.type = CP_ITEM_INT, .as.Int = 7}, B(0x47)},
	{{.type = CP_ITEM_INT, .as.Int = 42}, B(0x6a)},
	{{.type = CP_ITEM_INT, .as.Int = 528}, B(0x82, 0x82, 0x10)},
	{{.type = CP_ITEM_INT, .as.Int = 2147483647},
		B(0x82, 0xf0, 0x7f, 0xff, 0xff, 0xff)},
	{{.type = CP_ITEM_INT, .as.Int = 4294967295},
		B(0x82, 0xf1, 0x00, 0xff, 0xff, 0xff, 0xff)},
	{{.type = CP_ITEM_INT, .as.Int = 9007199254740991},
		B(0x82, 0xf3, 0x1f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff)},
	{{.type = CP_ITEM_INT, .as.Int = -9007199254740991},
		B(0x82, 0xf3, 0x9f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff)},
	{{.type = CP_ITEM_UINT, .as.UInt = 0}, B(0x00)},
	{{.type = CP_ITEM_UINT, .as.UInt = 1}, B(0x01)},
	{{.type = CP_ITEM_UINT, .as.UInt = 126}, B(0x81, 0x7e)},
	{{.type = CP_ITEM_UINT, .as.UInt = 127}, B(0x81, 0x7f)},
	{{.type = CP_ITEM_UINT, .as.UInt = 2147483647},
		B(0x81, 0xf0, 0x7f, 0xff, 0xff, 0xff)},
	{{.type = CP_ITEM_UINT, .as.UInt = 4294967295},
		B(0x81, 0xf0, 0xff, 0xff, 0xff, 0xff)},
	{{.type = CP_ITEM_DOUBLE, .as.Double = 223.0},
		B(0x83, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe0, 0x6b, 0x40)},
	{{.type = CP_ITEM_DECIMAL, .as.Decimal = {.mantisa = 23, .exponent = -1}},
		B(0x8c, 0x17, 0x41)},
	...
	{{.type = CP_ITEM_DATETIME,
		...
		B(0x8d, 0x82, 0x11)},
	{{.type = CP_ITEM_STRING, .rchr = "", .as.String = {.flags = CPBI_F_SINGLE}},
		B(0x86, 0x00)},
	{{.type = CP_ITEM_STRING,
		 .rchr = "foo",
	...
	{{.type = CP_ITEM_LIST}, B(0x88)},
	{{.type = CP_ITEM_MAP}, B(0x89)},
	{{.type = CP_ITEM_IMAP}, B(0x8a)},
	{{.type = CP_ITEM_META}, B(0x8b)},
	{{.type = CP_ITEM_CONTAINER_END}, B(0xff)},
};
